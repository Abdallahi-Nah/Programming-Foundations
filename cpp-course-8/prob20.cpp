...
sDate returnDate(sDate dateFrom, int vacationDays) {
    sDate dateTo = dateFrom;
    for(int i=1; i <= vacationDays; i++) {
        if(!isItBuisnessDay(dateFrom)) {
            dateTo = dateAfterAddingOneDay(dateTo);
        } 
        dateFrom = dateAfterAddingOneDay(dateFrom);
        dateTo = dateAfterAddingOneDay(dateTo);
    }
...
int main() {
    ...
    date1 = readDate();
    int vacationDays = readNumber();
    date2 = returnDate(date1, vacationDays);

    printDate("Return date", date2);
